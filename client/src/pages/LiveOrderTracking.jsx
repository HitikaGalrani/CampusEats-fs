...
export const LiveOrderTracking = () => {
  ...
  const activeLiveOrder = useSelector(
    (state) => state.order.activeLiveOrder
  );
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          'Failed to load order for tracking'
      );
    } finally {
      setLoading(false);
    }
  ...
  // Get order and join Socket.IO room
  useEffect(() => {
    ...
    if (targetId) {
      ...
      api
        ...
        .catch(() => {
          setError('Failed to retrieve active order.');
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  useEffect(() => {
    const handleStatusUpdate = (payload) => {
      console.log(
        '⚡ [Socket.IO] Received orderStatusUpdated:',
        payload
      );
      ...
      if (order && order._id === payload.orderId) {
        setOrder((prev) => ({
          ...
          estimatedTime:
            payload.estimatedTime || prev.estimatedTime,
        }));
    ...
    const handleOrderReady = (payload) => {
      console.log(
        '⚡ [Socket.IO] Received orderReady event!'
      );
